var GameModeNormal = {
	...
	startGame: function()
	{
		...
		debugConsole.log("目标持续时间: " + (targetDuration / 60));
	...
	endGame: function()
	{
		...
		document.getElementById("message").innerText = "若尚未泄精，游戏告负，速速收手！若已功成，善哉善哉！";
	...
	runStepFinish: function()
	{
		...
		if (cumFactor === 0.0)
		{
			...
			ProgressBarManager.cancelSlideProgress();
			State.updatePassType(State.STATE_DONE);
			document.getElementById("message").innerText = "恭贺你，已完成这番欲擒故纵之戏！咱们不日再战，情意绵绵。";
		...
		if (cumFactor < 0.0)
		{
			...
			document.getElementById("message").innerText = messages["noCum"][0].msg;
