...
var ImageBrowser = {
	...
	checkList: function()
	{
		...
		for (let i = 0; i < cache.length; ++i)
		{
			...
			{
				debugConsole.log("已损坏的 borwser 列表。重建...")
	...
	displayPrev: function()
	{
		if (State.bigImageIndex > 0)
		{
			...
			document.getElementById("buttonBigImageRemoveList").value = "从列表中删除";
			document.getElementById("buttonBigImageBlacklist").value = "黑名单图片";
	...
	openPicturePage: function(imgUrl) 
	{
		...
		{
			debugConsole.log("试图从 e621 获取 ID 以获取图片 " + imgUrl);
			...
			async function getPicsFromRequest(response)
			{
				...
				if (response.status < 200 || response.status > 299)
				{
					...
					{
						NotifMessage.displayError("无法打开此图片的 e621 页面。");
					}
					else
					{
				...
				{
					// MD5 Not found.
					NotifMessage.displayError("在 e621 上找不到图片。");
					return;
				}
